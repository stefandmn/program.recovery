...
class SystemRecovery:
	...
	def backup(self):
		...
		if self.remoteFS.exists(self.remoteFS.RootPath):
			...
			commons.debug("Remote path exists - may have old files in it", "SystemRecovery")
		else:
		...
		if not self._createValidationFile():
			...
			commons.error("Validation file can not be created, backup process skipped")
			self.status = -1
			return
		commons.debug("Creating files list", "SystemRecovery")
		allFiles = []
		...
		for fileGroup in allFiles:
			...
			if not filesCopied:
				commons.warn("Not all files were copied: %s" %self.localFS.RootPath, "SystemRecovery")
				self.status += 1
		...
		if commons.setting("compress_backups"):
			...
			if not fileCopied:
				...
				commons.warn("The destination may not be writeable: %s" %self.remoteFS.RootPath)
				self.ststus = -1
				return
	...
	def restore(self):
		...
		if not self._checkValidationFile(self.remoteFS.RootPath):
			commons.error("Validation file can not be validated, restore process skipped")
			self.status = -1
			return
		commons.debug("Creating files list", "SystemRecovery")
		allFiles = []
	...
	def run(self, mode=-1):
		result = True
		if not commons.any2bool(xbmc.getInfoLabel("Window(%s).Property(%s)" % (10000, "PluginRecovery.Running"))):
			...
			if self.remoteFS.RootPath is not None and self.remoteFS.RootPath != '':
				commons.debug("Local directory: " + self.localFS.RootPath + ", Remote directory: " + self.remoteFS.RootPath, "SystemRecovery")
				if mode == self.Backup:
					if commons.setting("compress_backups"):
			...
			commons.warn('Script already running, no additional instance is needed')
			result = False
		return result

	def backupFiles(self, fileList, source, dest):
		result = True
		commons.debug("Writing files to '" + dest.RootPath + "', Source is '" + source.RootPath + "'", "SystemRecovery")
		for aFile in fileList:
			commons.debug('Writing file: ' + aFile, "SystemRecovery")
			if aFile.startswith("-"):
	...
	def _rotateBackups(self):
		total_backups = commons.setting('backup_rotation')
		if total_backups > 0:
			...
			if len(dirs) > total_backups:
				...
				while remove_num < (len(dirs) - total_backups):
					commons.debug("Removing backup " + dirs[remove_num][0], "SystemRecovery")
